...
class HorosSerial(serializers.ModelSerializer):
    class Meta:
        model = Horas
        fields = ["id", "hora"]
...
class AgendasSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        data["horario"] = list(
            filter(
                lambda hora: valida_datas(data["dia"], hora, data["id"]),
                data["horario"],
            )
        )
...
class ConsultaSerializer(serializers.ModelSerializer):
    ...
    agenda_id = serializers.PrimaryKeyRelatedField(
        write_only=True, queryset=Agendas.objects.all()
    )
    ...
    def create(self, validated_data):
        ...
        expirado = Validators().data_expirada(agenda, horario.hora)
        if expirado:
            raise (
        ...
        hora_agenda_existe = Validators().check_agenda(agenda.pk, horario.pk)
        if not hora_agenda_existe:
            raise (serializers.ValidationError("Agenda e hora não coincidem"))

        agenda_em_uso = Validators().consulta_disponivel(horario.pk, agenda.pk)
        if agenda_em_uso:
            raise (
                serializers.ValidationError(
        ...
        mesma_datahora = Validators().usuario_tem_mesma_datahora(
            usuario.pk, horario.pk, agenda.dia
        )
